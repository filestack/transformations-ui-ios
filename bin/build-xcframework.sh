...
xcodebuild archive -scheme $SCHEME -configuration Release -sdk iphoneos14.0 -destination "generic/platform=iOS" -archivePath $DEST/$FRAMEWORK.iphoneos.xcarchive OBJROOT=build/iOS | xcpretty
# Archive using iPhoneSimulator SDK.
xcodebuild archive -scheme $SCHEME -configuration Release -sdk iphonesimulator14.0 -destination "generic/platform=iOS Simulator" -archivePath $DEST/$FRAMEWORK.iphonesimulator.xcarchive OBJROOT=build/simulator | xcpretty
# Create XCFramework.
